...
```{r os_development, fig.height = 10, fig.width = 5.5}
plot_time(df, var_overview, os, type = "agreement")
```

### Reporting and Academic Integrity

```{r raia_development, fig.height = 10, fig.width = 5.5}
plot_time(df, var_overview, raia, type = "agreement")
```

```{r raif_development, fig.height = 7, fig.width = 5.5}
plot_time(df, var_overview, raif, type = "frequency")
```

## Education

```{r edua_development, fig.height = 4.5, fig.width = 5.5}
plot_time(df, var_overview, edua, type = "agreement")
```

```{r eduf_development, fig.height = 4, fig.width = 5.5}
plot_time(df, var_overview, eduf, type = "frequency")
```

## Institution/Administration

```{r adm_development, fig.height = 6, fig.width = 5.5}
