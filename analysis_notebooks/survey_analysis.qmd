...
```{r os_overview, fig.height = 4.5, fig.width = 6.5}
...
```{r raia_overview, fig.height = 4.5, fig.width = 6.5}
...
```{r raif_overview, fig.height = 3.6, fig.width = 6.5}
...
```{r edua_overview, fig.height = 2.7, fig.width = 6.5}
...
```{r eduf_overview, fig.height = 2.6, fig.width = 6.5}
...
```{r adm_overview, fig.height = 3, fig.width = 6.5}
...
```{r os_development, fig.height = 6.5, fig.width = 7}
os_qs <- df %>% 
...
# get titles in
titles <- var_overview %>% 
  select(var_id, var_short)

os_qs <- os_qs %>% 
  left_join(titles, by = join_by(var == var_id))

ggplot(os_qs, aes(x = year, y = perc, color = value)) + 
    geom_line(size = 0.8) +
    geom_point() +
  facet_wrap(vars(var_short), ncol = 3) +
    labs(x = "Survey Year", y = "Proportions", color = "Responses") +
...
# maybe better to sort similar to the overview plot? so sort by agreement

```

```{r os_development_old, fig.width=8, fig.height=15}
plots <- list()
...
for (q in raif) {
  plot <- plot_frequency_line(df, question = q, legend = TRUE, ylim = 70)
  plots[[q]] <- plot
}
...
for (q in adm) {
  plot <- plot_frequency_line(df, question = q, legend = TRUE, ylim = 75)
  plots[[q]] <- plot
}
...
plot_agreement_groups(students, "X15", "X8", legend_reverse = FALSE)
```
