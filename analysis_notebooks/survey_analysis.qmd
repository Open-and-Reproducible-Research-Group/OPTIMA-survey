...
groups <- df %>% 
  ...
  mutate(X52d = case_when(
    str_detect(X52, "\\sagree$") ~ "agree",
    str_detect(X52, "\\sdisagree$") ~ "disagree",
    is.na(X52) ~ "NA",
    TRUE ~ X52)) %>% 
  # Filter to calculate proportions of those that knew a response
  filter(X52d %in% c("agree", "disagree")) %>% 
  group_by(X8, X64) %>% 
  summarise(perc = sum(X52d == "agree") / n()) %>% 
  ungroup() %>% 
  mutate(X8 = fct_recode(X8,
                         "doctoral student - ScD" =
                           "doctoral student - ScD (higher doctorate)"))
...
```{r}
ggplot(students, aes(x = X64, y = perc, color = X8)) + 
    geom_line(size = 0.8) +
    geom_point() +
    labs(x = "Survey Year", y = "Agreement", color = "Academic Role") +
...
```{r}
ggplot(staff, aes(x = X64, y = perc, color = X8)) + 
    geom_line(size = 0.8) +
    geom_point(shape = 15) +
    labs(x = "Survey Year", y = "Agreement", color = "Academic Role") +
    scale_x_continuous(breaks = c(2021, 2022, 2023)) +
...
```{r}
ggplot(groups, aes(x = X8, y = perc, fill = as.factor(X64))) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  geom_hline(yintercept = 0) +
  labs(x = "Academic Role", y = "Agreement", fill = "Survey Year") +
