...
# Could be rewritten to group by any demographic characteristic
plot_agreement_years <- function(df, var_overview, question) {
  ...
  p2 <- p_nas %>% 
    ggplot(aes(y = as.factor(X64), x = n/100)) +
    geom_col(fill = "grey50", width = .7) +
...
```{r}
plot_agreement_years(df, var_overview, "X46")
```

```{r}
plot_frequency_years(df, var_overview, "X30")
```
