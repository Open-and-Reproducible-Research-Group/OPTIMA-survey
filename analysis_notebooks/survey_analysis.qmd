...
plot_frequency_overview <- function(df, var_overview, columns) {
  ...
  pdata <- step1 %>% 
    ...
    group_by(var) %>%
      mutate(prop = n/sum(n),
             order = case_when(
               str_detect(val, "\\bvery often\\b|\\bfrequently\\b") ~ prop,
               TRUE ~ 0),
             order = sum(order))
...
plot_agreement <- function(
  ...
  # title <- str_wrap(var_overview$var_full[var_overview$var_id == question], width = 110)

  p1 <- pdata %>% 
    ggplot(aes(as.factor(.data[[group]]), prop, fill = val)) +
    geom_chicklet(width = .7) +
    coord_flip() +
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = c("steelblue", "olivedrab4", "goldenrod2", "indianred")) +
  ...
  p2 <- p_nas %>% 
    ggplot(aes(y = as.factor(.data[[group]]), x = n)) +
    geom_col(fill = "grey50", width = .7) +
...
```{r}
plot_agreement(df, var_overview, "X45", group = "X64", filter = "X8", filter_val = "university administrator")

knitr::kable(table_answers(df, "X45", "X64", "X8", "university administrator"), format = "html")
```
...
```{r}
plot_frequency(df, var_overview, "X26", group = "X64")

knitr::kable(table_answers(df, "X26", "X64"), format = "html")
```
