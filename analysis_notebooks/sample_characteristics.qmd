...
df <- targets::tar_read(processed_data)
```
...
```{r}
df$X53 <- factor(df$X53, levels = c("strongly disagree", "rather disagree", "rather agree", "strongly agree"))
...
subset_df <- subset(df, duration < 3 & X53 %in% c("strongly disagree", "rather disagree", "NA"))

print(paste("The number of respondents taking less than three minutes and indicating low clarity (or not answering) is", nrow(subset_df)))
```
